...
- ссылка на RFQ или описание возникшей ошибки сохраняется в локальной папке с письмом в отдельном файле `rfq_info.txt`

4. Подготовка шаблона ответного письма
- для сохраненных локально писем если нет флага `reply.txt` (ответ был составлен ранее) и есть флаг `rfq_info.txt` (есть ответ от API - с ошибкой или ссылкой) создается шаблон ответного письма `reply.txt`

5. Отправка ответа
- для сохраненных локально писем если нет флага `reply_sent` (ответ направлен ранее) или `do_not_process` (недостаточно информации для создания FRQ) берется шаблон ответного письма `reply.txt` и на основе метаинформации соответствующего входящего писма отсылаются ответные письма отправителям
