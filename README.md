...
flowchart TD
    ...
    O -- Нет --> Q[Сформировать RfqCreateRequest]

    Q --> R[Найти RFQ по наименованию]

    R --> S{"RFQ уже существует?"}

    S -- Да --> S1[Зафиксировать найденный RFQ]

    S -- Нет --> T[Выполнить запросы к API 7Rights]

    T --> U{"Все запросы успешны?"}

    U -- Да --> V[Получить RFQ ID и сформировать ссылку]

    U -- Нет --> W[Сохранить ошибки API]

    X[Сохранить результат в rfq_info.json]
    ...
    Y{"Файл reply уже существует?"}

    Y -- Да --> Z[Пропустить]

    Y -- Нет --> AA{"Есть rfq_info.json?"}
